...
prompt_status() {
  local symbols
  symbols=()
  [[ $RETVAL -ne 0 ]] && symbols+="%{%F{white}%}$CROSS"
  [[ $UID -eq 0 ]] && symbols+="%{%F{yellow}%}$LIGHTNING"
  [[ $(jobs -l | wc -l) -gt 0 ]] && symbols+="%{%F{cyan}%}$GEAR"
  [[ -n "$symbols" ]] && prompt_segment "$STATUS_BG" default " $symbols "
}

prompt_dir() {
  prompt_segment "$DIR_BG" yellow " %~ "
}
...
prompt_context() {
  ...
  if [[ "$user" != "$DEFAULT_USER" || -n "$SSH_CONNECTION" ]]; then
    prompt_segment "$CONTEXT_BG" "$PRIMARY_FG" " $user@%m "
  fi
}
